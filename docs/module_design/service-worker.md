...
`chrome.runtime.onInstalled` の `reason`（`install` / `update`）で処理を分けない。初回インストールと更新とで `setPanelBehavior` の設定内容は同一であり、分ける理由がない。

分岐を置く候補として**保存レコードの移行**があった。レコードはバージョン欄を持つため（[ADR 0013](../ADR/0013-persist-code-in-storage-local.md)）、いずれ古い形を読み替える処理が要る。これを `reason === "update"` の分岐に置く案は採らない。

移行は `main.js` の `restoreState` が行う（[main.md](main.md)）。理由は 3 つある。

1. **Service Worker はいつでも停止する。** 移行の途中で止まれば、中途半端に書き換わったレコードが残る。
2. **移行が要るかどうかは読んだ時点で分かる。** `restoreState` は既にバージョン欄を見ており、判断の材料がその場にある。読む側が直せば、書き換えの契機が 1 箇所に収まる。
3. **`onInstalled` は走らない経路がある。** 開発中に `dist/` を差し替えて拡張をリロードした場合など、バージョンが上がっても通らないことがある。移行をここに賭けると、通らなかったときに古い形のまま読まれる。

### 初回インストール時にサイドパネルを開かない

拡張を入れた直後に自動でサイドパネルを開くことはしない。

**`chrome.sidePanel.open()` はユーザージェスチャを要求する。** `onInstalled` はジェスチャの文脈にないため、そもそも呼べない。設計上の好みではなく API の制約で決まる。

仮に呼べたとしても、どのタブに対して開くかという問題が残る。入口の案内は `manifest.json` の `action.default_title` に持たせる。
